...
# Licence : Apache 2.0
#
# Description : controleur HTTP des studios de la Bibliotheque publique.
...
class StudioController:
    """Enregistre les routes HTTP liees aux studios globaux."""

    PUBLIC_ENDPOINTS = frozenset({"list_library_studios"})

    def __init__(self, library_service_factory=None, library_query_parser=None):
        """Initialise le controleur des studios.
    ...
    def register_routes(self, flask_app: Flask) -> None:
        """Enregistre les routes studios dans l'application Flask.
        ...
        flask_app.add_url_rule(
            "/api/library/studios",
            endpoint="list_library_studios",
            view_func=self._as_view(self.list_library_studios),
            methods=["GET"],
        )
    ...
    def list_library_studios(self):
        """Liste les studios publics de la Bibliotheque.

        Args:
            Aucun.

        Returns:
            flask.Response | tuple[flask.Response, int]: Studios JSON ou erreur JSON.
        """
