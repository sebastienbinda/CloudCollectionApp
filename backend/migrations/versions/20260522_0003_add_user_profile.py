...
# Licence : Apache 2.0
#
"""Ajoute le profil applicatif aux utilisateurs."""
...
def upgrade() -> None:
    """Ajoute la colonne `profile` a `t_user`.
    ...
    op.execute(
        ...
        ADD COLUMN IF NOT EXISTS profile VARCHAR(16) NOT NULL DEFAULT 'USER'
        """
    )
    op.execute(
        f"""
        ...
        BEGIN
            IF NOT EXISTS (
                ...
                WHERE conname = 'ck_t_user_profile'
                AND nspname = '{schema_name}'
            ) THEN
                ALTER TABLE "{schema_name}".t_user
                ADD CONSTRAINT ck_t_user_profile
...
def downgrade() -> None:
    """Supprime la colonne `profile` de `t_user`.
    ...
    op.execute(
        f"""
        ALTER TABLE "{schema_name}".t_user
        DROP CONSTRAINT IF EXISTS ck_t_user_profile,
