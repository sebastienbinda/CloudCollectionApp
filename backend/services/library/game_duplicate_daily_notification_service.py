...
# Licence : Apache 2.0
#
# Description : notification quotidienne des jeux signales comme doublons.
...
class GameDuplicateDailyNotificationService:
    """Compte les doublons signales et notifie l'administrateur si besoin."""
    ...
    def __init__(
        ...
        """Initialise le service de notification quotidienne des doublons.
        ...
        if engine is None and not configuration.is_database_enabled():
            raise ValueError("DATABASE_URL est requis pour notifier les doublons.")
    ...
    @classmethod
    def from_environment(cls) -> "GameDuplicateDailyNotificationService":
        ...
        Returns:
            GameDuplicateDailyNotificationService: Service configure.
        """
    ...
    def default_template_path(cls) -> Path:
        ...
        return (
            ...
            / "game_duplicate_daily_notification_template.txt"
        )
    ...
    def notify_if_duplicates_exist(self) -> int:
        """Envoie un email administrateur si des doublons sont a traiter.

        Args:
            Aucun.

        Returns:
            int: Nombre de jeux signales comme doublons.
        """

        duplicate_count = self._count_reported_duplicates()
        if duplicate_count <= 0:
            return duplicate_count
        if not self.admin_notification_email:
            self.logger.warning(
                "ADMIN_NOTIFICATION_EMAIL absent: notification quotidienne doublons ignoree."
            )
            return duplicate_count
        ...
        sender.send_email(
            ...
            subject="Jeux en doublon a traiter",
            body=self._build_email_body(duplicate_count),
        )
        return duplicate_count

    def _count_reported_duplicates(self) -> int:
        with self.engine.connect() as connection:
            return self.repository.count_reported_duplicates(connection)

    def _build_email_body(self, duplicate_count: int) -> str:
        library_games_url = f"{self.site_url}/bibliotheque/jeux?duplicate_flag=true"
