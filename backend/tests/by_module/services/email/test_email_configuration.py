...
class EmailConfigurationTest(unittest.TestCase):
    ...
    def test_generic_smtp_configuration_reads_password_file(self):
        """Verifie la lecture de `SMTP_PASSWORD_FILE`.

        Args:
            Aucun.

        Returns:
            None: Les assertions valident le mot de passe SMTP charge.
        """

        with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as secret_file:
            secret_file.write("file-password\n")
        ...
        try:
            with patch.dict(
                os.environ,
                {
                    ...
                    "SMTP_PASSWORD_FILE": secret_file_path,
                },
                clear=True,
            ):
                configuration = EmailConfiguration.from_environment()
        ...
        self.assertEqual("file-password", configuration.smtp_password)
