...
print_usage() {
  echo "Usage: ./scripts/test_email.sh --to adresse@example.com [-p] [--subject 'Sujet'] [--body 'Message']"
...
if [ "$USE_ONLINE_STACK" = true ]; then
  COMPOSE_FILE="${PROJECT_ROOT}/runtime/docker-compose.online.yml"
fi
...
docker compose -f "$COMPOSE_FILE" exec -T backend "${command_args[@]}" \
  < "${PROJECT_ROOT}/backend/scripts/send_test_email.py"
